...
streamings = db.query("SELECT DISTINCT streaming FROM series ORDER BY streaming ASC;")
...
puts '<section id="discoverStreams">'


(0...streamings.size).each do |j|
    series = db.query("SELECT imageName FROM series WHERE streaming = '" + streamings[j]['streaming'] + "';")
    series = series.to_a

    if (series.size >= 5)
    puts '<h3 class="text-white ms-4">' + streamings[j]['streaming'] + '</h3>'
